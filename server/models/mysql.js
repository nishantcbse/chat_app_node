...
sql.query("SELECT id FROM users", []).catch(async function (error) {
    console.log(error);
});
global.generateHash = (password)=> {
    return bcrypt.hashSync(password, bcrypt.genSaltSync(10), null);
};
global.validPassword = (password, existingpassword)=> {
    return bcrypt.compareSync(password, existingpassword);
}
global.currentTimeStamp = Date.now()
